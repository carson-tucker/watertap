...
def main():
    ...
    # feed information, feed concentration and recovery ranges to run
    ...
    # run parameter sweeps for feed concentration and recovery
    run_full_parameter_sweeps(analysis, cases, map_dir)

    ### Parameter sweep 2 ###
    #--- material factor sensitivity ---
    # example of a single-parameter sweep - can swap the parameter to vary
    case = {}
    case['name'] = 'case_1'
    case['w_f'] = 0.075
    case['rr'] = 0.5
    ...
    # parameter sweep
    param = 'material_factor' # param name points to dictionary made in get_param_var_dict()
    n_param = 41 # number of points in parameter sweep
    param_min = 3
    param_max = 9
...
def run_full_parameter_sweeps(analysis_file, cases, dir, system='mvc_full_opt'):
    # iterate over case studies and get correct folder name
...
def mvc_full_presweep(f_evap=5,
    ...
    # Fix case parameters to desired values
    if f_evap != 'vary': # vary means material factor varies with brine salinity
    ...
    m.fs.evaporator.U.fix(U_evap)
    m.fs.hx_distillate.overall_heat_transfer_coefficient[0].fix(U_d)
    m.fs.hx_brine.overall_heat_transfer_coefficient[0].fix(U_b)
    ...
    # evaporator and preheaters have same material factor
    mvc_full.add_evap_hx_material_factor_equal_constraint(m)
    if f_evap == 'vary': # add material factor dependence on brine salinity
