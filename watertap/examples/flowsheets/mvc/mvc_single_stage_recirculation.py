...
def build():
    ...
    m.fs.mixer_feed = Mixer(
        ...
        inlet_list=["hx_distillate_cold", "hx_brine_cold","recirculating_brine"],
    )
    m.fs.mixer_feed.pressure_equality_constraints[0, 2].deactivate()
    # Need to deactivate another pressure equality constraint
    ...
    # separator for recirculating brine
    m.fs.separator_brine = Separator(
        property_package=m.fs.properties_feed,
        outlet_list=["exiting_brine", "recirculating_brine"],
        split_basis=SplittingType.totalFlow,
    )
    ...
    m.fs.s10 = Arc(
        source=m.fs.evaporator.outlet_brine, destination=m.fs.separator_brine.inlet
    )
    m.fs.s11 = Arc(
        source=m.fs.separator_brine.recirculating_brine, destination=m.fs.pump_recirculation.inlet
    )
    m.fs.s12 = Arc(
        source=m.fs.pump_recirculation.outlet, destination=m.fs.mixer_feed.recirculating_brine
    )
    m.fs.s13 = Arc(
        source=m.fs.separator_brine.exiting_brine, destination=m.fs.pump_brine.inlet
    )
    m.fs.s14 = Arc(source=m.fs.pump_brine.outlet, destination=m.fs.hx_brine.hot_inlet)
    m.fs.s15 = Arc(source=m.fs.hx_brine.hot_outlet, destination=m.fs.brine.inlet)
    m.fs.s16 = Arc(source=m.fs.condenser.outlet, destination=m.fs.tb_distillate.inlet)
    m.fs.s17 = Arc(
